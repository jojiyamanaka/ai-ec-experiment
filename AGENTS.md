...
以下は過去のレビューで指摘された「task.md に書かれていない変更」の典型例。これらは **絶対に禁止**。

### ❌ 禁止: 既存コメントの削除

**例**: CartService.java で Javadoc やインラインコメントを削除
```java
// ❌ 禁止
- /**
-  * カートサービス
...
// ✅ 正しい: task.md に「削除」指示がなければコメントを維持
+ /**
+  * カートサービス
+  */
  @Service
...
### ❌ 禁止: UI要素の改変（SVG → テキスト化）

**例**: CartPage.tsx のアイコンをテキストに置換
```tsx
// ❌ 禁止
- <svg xmlns="..." viewBox="...">...</svg>
+ 削除

// ✅ 正しい: task.md に指示がなければSVGを維持
  <svg xmlns="..." viewBox="...">...</svg>
```

### ❌ 禁止: エラーメッセージ・文言の微調整

**例**: エラーメッセージを「わかりやすく」改変
```java
// ❌ 禁止
- "この商品は現在購入できません"
+ "この商品は現在公開されていません"

// ✅ 正しい: task.md のメッセージを一字一句そのまま使う
+ "この商品は現在購入できません"
```

### ❌ 禁止: コメントの「更新」改変

**例**: UpdateQuantityRequest.java のコメント
```java
// ❌ 禁止
- * 数量変更リクエスト
+ * 数量更新リクエスト

// ✅ 正しい: task.md に指示がなければ既存のまま
  * 数量変更リクエスト
```

### ✅ 許容される変更

- task.md に明記されたコード断片の追加
- task.md に「削除」と明記された箇所の削除
- 新規追加するimport文（該当グループの末尾に追加）
- task.md に記載された挿入位置への配置

---

## 実装チェックリスト

task.md を読んだ後、以下を自問してから実装すること:

- [ ] このタスクで追加するコードは、task.md に記載されているか？
- [ ] このタスクで削除するコードは、task.md に「削除」と明記されているか？
- [ ] 既存のコメント・UI・文言を変更しようとしていないか？
- [ ] task.md に書かれていない「改善」や「整形」をしようとしていないか？
- [ ] コード断片を「一字一句そのまま」使っているか？
