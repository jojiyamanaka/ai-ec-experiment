...
### デザインシステム（エディトリアルデザイン）

**デザインコンセプト**: モノトーン＋ニュアンスカラーによるエディトリアルデザイン。高級感と洗練性を重視し、視覚的なノイズを排除したミニマルなデザイン。

#### カラーパレット

**ベースカラー（Tailwind CSS 4 カスタム設定）**:
- **メインテキスト・ボタン**: `zinc-900` (#18181B)
- **補助テキスト**: `zinc-500` (#71717A)
- **境界線**: `stone-200` (#E7E5E4)
- **背景**: `stone-50` (#F5F5F4)
- **商品カード背景**: `stone-200` (#E7E5E4)

**廃止した色**:
- 青系（`blue-*`）、赤系（`red-*`）などの鮮やかな色
- 在庫バッジも `zinc-*` / `stone-*` 系に統一

**在庫状態バッジ**:
- 在庫あり: `zinc-700` / `stone-700`
- 残りわずか: `zinc-500` / `stone-400`
- 売り切れ: `zinc-400` / `stone-300`

#### タイポグラフィ

**フォントファミリー**:
- **見出し（Heading）**: `font-serif` — Playfair Display (Google Fonts)
- **本文（Body）**: `font-sans` — Inter (Google Fonts)
...
**サイズ**:
- 大見出し: `text-3xl` 以上
- 中見出し: `text-2xl`, `text-xl`
- 本文: `text-base`
- 補足・価格: `text-xs`, `text-sm`
...
**グリッドギャップ**:
- 商品カード間: `gap-6` (1.5rem) または `gap-x-6 gap-y-12`
...
- 文字間隔: `tracking-[0.2em]`
- パディング: `px-12 py-4`
- ホバー: `hover:bg-zinc-800`
- 角丸: 最小限（`rounded-sm` または `rounded-none`）

**ヘッダー**:
- 背景: `bg-white/80 backdrop-blur-md` (透過・ぼかし効果)
- 配置: `fixed` (固定配置)
