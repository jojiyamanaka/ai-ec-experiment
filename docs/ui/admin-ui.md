...
- 注文番号クリックでモーダル表示（`max-w-4xl`）
- 表示は読み取り専用のフォーム風レイアウトで、以下4セクションを表示
  - 注文情報: 注文番号, 注文日時, ステータス, 引当進捗
  - 会員情報・合計: 会員メール, 会員表示名, 合計金額
  - 注文明細: 商品名, 数量, 引当進捗, 小計
  - 返品情報（返品が存在する場合のみ表示）:
    - 返品ステータス: `RETURN_PENDING`「返品待ち」/ `RETURN_APPROVED`「返品承認済」/ `RETURN_CONFIRMED`「返品確定」/ `RETURN_CANCELLED`「返品キャンセル」
    - 返品理由（返品申請時の入力値）
    - 拒否理由（拒否時のみ表示）
    - 返品対象商品・数量の一覧
    - 返品操作ボタン（下記「返品操作」参照）
- ステータス変更ボタンはモーダル下部に配置し、状態に応じて表示を切り替える

### 返品操作

| 現在の状態 | アクション | 遷移先 |
|-----------|-----------|--------|
| RETURN_PENDING | 「承認」ボタン | RETURN_APPROVED |
| RETURN_PENDING | 「拒否」ボタン → 理由入力モーダル表示 | RETURN_CANCELLED |
| RETURN_APPROVED | 「確定」ボタン | RETURN_CONFIRMED |
| RETURN_APPROVED/RETURN_CONFIRMED/RETURN_CANCELLED | — | 操作不可 |
