...
**ビジネスルール**:
- 売り切れ商品はカートに追加不可
- 1商品あたりの数量は最小1個、最大9個まで
- 数量を 0 にすると自動的に削除
- 非公開商品（`isPublished = false`）はカートから自動除外
- 合計金額はリアルタイムで再計算
...
**ビジネスルール**:
- 注文番号は `ORD-xxxxxxxxxx` 形式で自動生成
  - xxxxxxxxxx: 0埋め10桁の連番（例: 0000000001）
- 注文確定時にカートを自動クリア
- 注文時に在庫引当を実施（仮引当 → 本引当）
...
- ゲストユーザーのカート・注文を識別するために使用

### 認証トークン（会員ユーザー、CHG-006）
- ログイン時に発行される（UUID v4）
- `localStorage` に保存（キー: `authToken`）
- APIリクエストに `Authorization: Bearer <token>` ヘッダーで送信
- 有効期限: 7日間
- 会員の認証・認可に使用
...
#### ゲストユーザー
- カート: セッションID (`sessionId`) でスコープ、`userId = null`
- 注文: セッションID (`sessionId`) でスコープ、`userId = null`

#### 会員ユーザー
- カート: 会員ID (`userId`) でスコープ
- 注文: 会員ID (`userId`) でスコープ
- 注文履歴: 会員IDで取得可能
...
- **セッションID**: 有効期限なし（永続保存）
- **認証トークン**: 7日間
- **Phase 2以降**: セッション有効期限の実装予定

**参照**:
- 技術仕様: [SPEC.md](../SPEC.md) - セッション管理・認証・認可
- データモデル: [data-model.md](./data-model.md) - User, AuthToken エンティティ
- 認証仕様: [specs/authentication.md](./specs/authentication.md)
