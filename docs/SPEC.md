...
- **記録対象**:
  ...
  - `LOGIN_FAILURE`: ログイン失敗（認証エラー）
  - `AUTHORIZATION_ERROR`: 認可エラー（権限不足）
  - `ADMIN_ACTION`: 管理者操作（商品編集、注文ステータス変更等）
- **トランザクション**: `REQUIRES_NEW`（操作履歴は必ず記録）

#### 会員とゲストの共存
- **ゲストユーザー**: `userId = null`、`sessionId` でカート・注文を識別
- **会員ユーザー**: `userId` でカート・注文を識別
- **カート引き継ぎ**: ログイン時にゲストカートを会員カートにマージ（Task3で実装予定）

**参照**: [data-model.md](./data-model.md) - User, AuthToken, OperationHistory エンティティ定義
...
2. **認証・認可**: トークンベース認証とロールベースアクセス制御を実装済み（CHG-006）。会員・ゲスト共存。
...
1. **カート引き継ぎ**: ログイン時にゲストカートを会員カートにマージ（CHG-006 Task3で実装予定）
