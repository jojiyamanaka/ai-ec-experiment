# CHG-002: 非公開商品のカート整合性（設計）

要件: `docs/01_requirements/CHG-002_非公開商品カート整合性.md`
作成日: 2026-02-12
...
既存の例外処理パターン（`BusinessException` / `GlobalExceptionHandler`）とエラーレスポンス形式（`ApiResponse.error()` / `ApiResponse.errorWithDetails()`）を踏襲する。CHG-001（在庫不足詳細エラー）で導入した `details` 付きエラーレスポンスの仕組みを再利用する。
