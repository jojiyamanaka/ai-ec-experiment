...
6. ホストから `localhost` 到達不可の場合は、対象コンテナ内から `curl` を実行する（customer は `customer-bff`、admin は `backoffice-bff`）
...
```bash
bash scripts/repair_flyway_checksum.sh
```

必要に応じて接続先を上書きする（例）:

```bash
FLYWAY_URL="jdbc:postgresql://postgres:5432/ec_db" \
FLYWAY_USER="ec_user" \
FLYWAY_PASSWORD="ec_password" \
bash scripts/repair_flyway_checksum.sh
```
