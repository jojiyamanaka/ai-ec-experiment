...
- T-1: 新規テーブル不使用・`shipment_type = RETURN` 流用は設計書 §1.1 と一致。`orders.delivered_at` の扱い（NULL のまま → `RETURN_PERIOD_EXPIRED`）は設計書 §1.6 と一致。E2E 参照データ絞り込みは設計外の実装判断だが、契約（API・境界）に影響しない。
- T-2: `ReturnUseCase` の package-private 実装は設計書 §4（ArchUnit ルール5）・§5.1 と一致。`statusLabel` を Core API 側 DTO で付与する方針は設計書 §2.1 と一致。Port シグネチャ（`ReturnCommandPort` / `ReturnQueryPort`）は設計書 §5.1 の契約と一致。
- T-3: Customer / BackOffice BFF の薄い透過構成は設計書 §2.2 / §2.3 と一致。共有 DTO を `bff/shared` に集約は設計書 §4 と一致。

### リスクと未解決の評価

- **`vitest` localStorage 警告フォールバック**: テストセットアップでメモリ実装フォールバックを入れて吸収済み。全テスト PASS しており対応不要。実行環境依存の問題のため将来的な改善余地はあるが、今回スコープ外。
- **ローカル E2E データの返品状態**: 手動確認による状態遷移のみ。ローカル開発環境限定で本番・CI に影響なし。対応不要。

### テスト結果の評価

テスト結果に FAIL 項目なし。全 PASS のため対応不要。
