...
- **薄いプロキシ**: BFF はビジネスロジックを持たず、Core API へのプロキシに徹する
...
| /api/products | GET | 商品一覧取得 | 不要 | GET /api/item |
| /api/products/:id | GET | 商品詳細取得 | 不要 | GET /api/item/:id |
...
- **トークン検証**: `auth.guard.ts` が Core API の `/api/auth/me` を呼び出してトークンを検証
- **セッションID**: `X-Session-Id` ヘッダーをフロントエンドから受け取り、Core API に転送
