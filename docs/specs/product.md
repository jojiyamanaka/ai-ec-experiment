...
| フィールド | 型 | 説明 |
|-----------|----|------|
| `productCode` | String | 品番（ユニーク） |
| `name` | String | 商品名 |
| `description` | String | 説明 |
| `categoryId` | Long | 商品カテゴリID |
| `categoryName` | String | 商品カテゴリ名（表示用） |
| `price` | Integer | 価格（円、税込、整数） |
| `stock` | Integer | 在庫数 |
| `isPublished` | Boolean | 商品公開フラグ |
| `publishStartAt` / `publishEndAt` | Instant | 公開期間 |
| `saleStartAt` / `saleEndAt` | Instant | 販売期間 |
...
| 判定 | 条件 |
|------|------|
| 顧客向け表示可否 | `product.isPublished && category.isPublished && now ∈ [publishStartAt, publishEndAt]` |
| 購入可否 | `顧客向け表示可否 && now ∈ [saleStartAt, saleEndAt] && stock > 0` |

### 期間制約

- 公開期間: `publishStartAt <= publishEndAt`（両方指定時）
- 販売期間: `saleStartAt <= saleEndAt`（両方指定時）
- 販売期間は公開期間内に収まること

違反時は `INVALID_SCHEDULE` を返却。
...
- カテゴリは `product_categories` で管理（`name`, `displayOrder`, `isPublished`）
- `isPublished = false` のカテゴリは商品登録/更新で指定不可（`CATEGORY_INACTIVE`）
- カテゴリ非公開時、所属商品は顧客向け一覧/詳細から非表示
